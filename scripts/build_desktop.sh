...
case "$PACKAGE_TARGET" in
  dist | --dist)
    ...
    OUTPUT_MESSAGE="AppImage: $DESKTOP_DIR/release/Six Degrees of Anime-0.1.0.AppImage"
    ;;
  linux | --linux)
    PACKAGE_SCRIPT="dist:linux"
    OUTPUT_MESSAGE="AppImage: $DESKTOP_DIR/release/Six Degrees of Anime-0.1.0.AppImage"
...
if [[ "$PACKAGE_SCRIPT" == *":win" && "$(uname -s)" != MINGW* && "$(uname -s)" != MSYS* && "$(uname -s)" != CYGWIN* ]]; then
  echo "Windows desktop packaging must be run on Windows so PyInstaller can create anime-six-degrees-backend.exe." >&2
  exit 1
fi
